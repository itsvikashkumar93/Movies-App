...
function SideNav() {
  ...
  return (
    <div className="h-screen w-[100%] border-r-[0.1vw] border-zinc-500 py-8 px-4 bg-[#1F1E24]">
      ...
      <nav className="">
        <h1 className="text-white text-xl mt-3 mt-2 sm:mt-8 sm:mb-4 font-semibold py-3 px-3 ">
          New Feeds
        </h1>
        {navData.map((elem, i) => (
          <Link
      ...
      <nav className="">
        <h1 className="text-white text-xl sm:mb-4 font-semibold py-3 px-3 ">
          Website Information
        </h1>
        {navData2.map((elem, i) => (
          <Link
            className="sm:text-lg block text-zinc-300 py-3 px-3 sm:mb-2 rounded hover:bg-[#6556CD] hover:text-zinc-100 duration-300"
            key={i}
          >
            <i className={`${elem.icon} mr-2`}></i>
            {elem.title}
